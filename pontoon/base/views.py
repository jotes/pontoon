...
def translate(request, locale, slug, part):
    ...
    if waffle.flag_is_active(request, "translate_next"):
        url = reverse(
            ...
            kwargs={"project": slug, "locale": locale, "resource": part,},
        )
...
def translate_locale_agnostic(request, slug, part):
    ...
    if user.is_authenticated():
        ...
        if locale and project_locales.filter(code=locale).exists():
            path = reverse(
                "pontoon.translate", kwargs=dict(slug=slug, locale=locale, part=part)
            )
            return redirect("%s%s" % (path, query))

    locale = utils.get_project_locale_from_request(request, project_locales)
    path = (
        reverse("pontoon.translate", kwargs=dict(slug=slug, locale=locale, part=part))
        if locale
...
def get_translations_from_other_locales(request):
    ...
    translations = Translation.objects.filter(
        entity=entity, locale__in=locales, plural_form=plural_form, approved=True
