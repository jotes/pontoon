...
from pontoon.checks.utils.compare import (
...
def test_unsupported_resource_file():
    ...
    with pytest.raises(UnsupportedResourceTypeError):
        cast_to_compare_locales('.random_ext', None, None, None)


@pytest.mark.django_db
def test_cast_to_properties_singular(entity_with_comment, translation0):
    assert cast_to_compare_locales('.properties', entity_with_comment, translation0.string, 0) == (
        ComparePropertiesEntity(
            'KeyEntity0',
            'entity0',
            'entity0',
...
@pytest.mark.django_db
def test_cast_to_properties_plural(plural_entity, plural_translation):
    assert cast_to_compare_locales('.properties', plural_entity, plural_translation.string, 1) == (
        ComparePropertiesEntity(
            'KeyEntity0',
            'plural entity0',
            'plural entity0',
        ...
        ComparePropertiesEntity(
            'KeyEntity0',
            'Plural translation for entity0',
            'Plural translation for entity0',
            CommentEntity('example comment'),
        )
    )
...
def test_valid_translations(quality_check_args):
    ...
    """
    assert quality_check(**quality_check_args) == {}
    ...
    (
        ...
        (
            mock_quality_check_args(
                resource_ext='properties',
                string='%s Foo %s bar %s %',
                string_plural='Plural setting  %s \p',
                translation='%d Bar %d foo %d',
                plural_form=1
            ),
            {
                'clErrors': [u'argument 1 `d` should be `s`']
            }
        ),
        (
            mock_quality_check_args(
...
def test_invalid_properties_translations(quality_check_args, failed_checks):
    assert quality_check(**quality_check_args) == failed_checks
